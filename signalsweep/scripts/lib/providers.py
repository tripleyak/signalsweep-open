...
ANTHROPIC_DEFAULT = "claude-opus-4-6"
...
class AnthropicClient(ReasoningClient):
    name = "anthropic"
...
_MODEL_DEFAULTS: dict[str, tuple[str, str]] = {
    ...
    "anthropic": (ANTHROPIC_DEFAULT, ANTHROPIC_DEFAULT),
    "xai": (XAI_DEFAULT, XAI_DEFAULT),
    "openrouter": (OPENROUTER_DEFAULT, OPENROUTER_DEFAULT),
}


def _config_first(config: dict[str, Any], *keys: str) -> str | None:
    for key in keys:
...
def _provider_model_pin_key(provider_name: str) -> str | None:
    return {
        "openai": "OPENAI_MODEL_PIN",
        "anthropic": "ANTHROPIC_MODEL_PIN",
        "xai": "XAI_MODEL_PIN",
        "gemini": "GEMINI_MODEL_PIN",
        "openrouter": "OPENROUTER_MODEL_PIN",
    }.get(provider_name)
...
def _resolve_model_pins(config: dict[str, Any], depth: str, provider_name: str) -> tuple[str, str]:
    ...
    provider_pin_key = _provider_model_pin_key(provider_name)
    provider_pin = _config_first(config, provider_pin_key) if provider_pin_key else None
    shared_pin = _config_first(config, "SIGNALSWEEP_MODEL_PIN") or provider_pin
    planner_model = (
        _config_first(config, "SIGNALSWEEP_PLANNER_MODEL", "LAST30DAYS_PLANNER_MODEL")
        or shared_pin
        or default_planner
    )
    rerank_model = (
        _config_first(config, "SIGNALSWEEP_RERANK_MODEL", "LAST30DAYS_RERANK_MODEL")
        or shared_pin
        or default_rerank
    )
...
def mock_runtime(config: dict[str, Any], depth: str) -> schema.ProviderRuntime:
    ...
    provider_name = (
        _config_first(config, "SIGNALSWEEP_REASONING_PROVIDER", "LAST30DAYS_REASONING_PROVIDER")
        or "gemini"
    ).lower()
...
def resolve_runtime(config: dict[str, Any], depth: str) -> tuple[schema.ProviderRuntime, ReasoningClient | None]:
    ...
    if provider_name == "auto":
        order = _provider_order(config)
        for candidate in order:
            if candidate == "gemini" and google_key:
                provider_name = "gemini"
                break
            if candidate == "openai" and openai_token and config.get("OPENAI_AUTH_STATUS") == env.AUTH_STATUS_OK:
                provider_name = "openai"
                break
            if candidate == "anthropic" and anthropic_key:
                provider_name = "anthropic"
                break
            if candidate == "xai" and xai_key:
                provider_name = "xai"
                break
            if candidate == "openrouter" and config.get("OPENROUTER_API_KEY"):
    ...
    if provider_name == "anthropic":
        if not anthropic_key:
            raise RuntimeError("Anthropic selected but ANTHROPIC_API_KEY is not configured.")
        runtime = schema.ProviderRuntime(
        ...
        return runtime, AnthropicClient(anthropic_key)
...
def _resolve_x_backend(config: dict[str, Any]) -> str | None:
    preferred = (
        _config_first(config, "SIGNALSWEEP_X_BACKEND", "LAST30DAYS_X_BACKEND")
        or ""
    ).lower()
    if preferred in {"xai", "bird", "xurl"}:
        return preferred
...
def _provider_order(config: dict[str, Any]) -> list[str]:
    raw = (
        _config_first(config, "SIGNALSWEEP_AUTO_PROVIDER_ORDER")
        or "gemini,openai,anthropic,xai,openrouter"
    )
    order = [part.strip().lower() for part in raw.split(",") if part.strip()]
    supported = {"gemini", "openai", "anthropic", "xai", "openrouter"}
    filtered = [provider for provider in order if provider in supported]
    for provider in ("gemini", "openai", "anthropic", "xai", "openrouter"):
...
_require_gemini_31_preview = _require_gemini_31
...
def extract_anthropic_text(payload: dict[str, Any]) -> str:
    ...
    if payload:
        print(f"[Providers] extract_anthropic_text: no text in payload keys: {list(payload.keys())}", file=sys.stderr)
    return ""
