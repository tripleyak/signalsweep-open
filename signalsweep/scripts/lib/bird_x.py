...
def install_bird() -> Tuple[bool, str]:
    ...
    if is_bird_installed():
        return True, "Bird search is bundled with /signalsweep v3.0.0 - no installation needed."
...
def _invoke_bird_subprocess(query: str, count: int, timeout: int):
    ...
    def _register(pid: int) -> None:
        ...
        try:
            from signalsweep import register_child_pid
    ...
    try:
        ...
        if pid_holder:
            try:
                from signalsweep import unregister_child_pid
