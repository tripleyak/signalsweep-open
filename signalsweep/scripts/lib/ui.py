...
MINI_BANNER = f"""{Colors.PURPLE}{Colors.BOLD}/signalsweep{Colors.RESET} {Colors.DIM}· researching...{Colors.RESET}"""
...
To fix this:
1. Add AUTH_TOKEN and CT0 to ~/.config/signalsweep/.env or .claude/signalsweep.env
...
class ProgressDisplay:
    ...
    def _show_banner(self):
        if IS_TTY:
            ...
            sys.stderr.write(f"/signalsweep · researching: {self.topic}\n")
        sys.stderr.flush()
...
def show_diagnostic_banner(diag: dict):
    ...
    if IS_TTY:
        ...
        lines.append(f"{Colors.DIM}│{Colors.RESET} {Colors.BOLD}/signalsweep v3.0.0 - Source Status{Colors.RESET}                 {Colors.DIM}│{Colors.RESET}")
        ...
        lines.append(f"{Colors.DIM}│{Colors.RESET}                                                     {Colors.DIM}│{Colors.RESET}")
        lines.append(f"{Colors.DIM}│{Colors.RESET}  Config: {Colors.BOLD}~/.config/signalsweep/.env{Colors.RESET}                  {Colors.DIM}│{Colors.RESET}")
        lines.append(f"{Colors.DIM}└─────────────────────────────────────────────────────┘{Colors.RESET}")
    else:
        # Plain text for non-TTY (Claude Code / Codex)
        lines.append("┌─────────────────────────────────────────────────────┐")
        lines.append("│ /signalsweep v3.0.0 - Source Status                 │")
        lines.append("│                                                     │")
        ...
        lines.append("│                                                     │")
        lines.append("│  Config: ~/.config/signalsweep/.env                  │")
        lines.append("└─────────────────────────────────────────────────────┘")
