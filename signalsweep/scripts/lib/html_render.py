"""HTML rendering for shareable signalsweep reports."""
...
CANONICAL_BOUNDARY_PATTERN = re.compile(r"\n?---\n# END OF signalsweep CANONICAL OUTPUT.*$", re.DOTALL)
...
<title>signalsweep · __TITLE__</title>
...
def _markdown_to_html(md: str) -> str:
    ...
    while index < len(lines):
        ...
        if stripped.startswith("🌐 signalsweep"):
            flush_paragraph()
            close_list()
...
def _protect_engine_footers(md: str) -> tuple[str, dict[str, str]]:
    ...
    def replace(match: re.Match[str]) -> str:
        token = f"__SIGNALSWEEP_ENGINE_FOOTER_{len(footers)}__"
...
def _wrap_engine_footer(body: str) -> str:
    ...
    return re.sub(
        r"__SIGNALSWEEP_ENGINE_FOOTER_\d+__",
        replace,
        body,
    )
...
def _build_colophon(report: schema.Report, *, topic: str | None = None) -> str:
    ...
    rerun = html.escape(f"/signalsweep {display_topic}")
    return (
        '<div class="colophon">\n'
        f"  Generated {generated} by /signalsweep v{html.escape(version)} · topic: {escaped_topic}<br>\n"
