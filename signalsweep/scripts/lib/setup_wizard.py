"""First-run setup wizard for signalsweep.
...
def run_auto_setup(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    results: Dict[str, Any] = {
        ...
        "env_written": False,
        "from_browser": "off" if public_mode else "auto",
...
def write_setup_config(env_path: Path, from_browser: str = "auto") -> bool:
    ...
    Args:
        env_path: Path to the .env file (e.g. ~/.config/signalsweep/.env)
...
def get_setup_status_text(results: Dict[str, Any]) -> str:
    ...
    if results.get("public_mode"):
        lines.append("  - Browser cookie scanning skipped in public mode")
    elif cookies_found:
