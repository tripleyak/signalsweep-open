...
def _ytdlp_ssh_host() -> Optional[str]:
    ...
    To use a value from ~/.config/signalsweep/.env, export it into the
    environment before invoking the engine, e.g. in a wrapper:
        set -a; source ~/.config/signalsweep/.env; set +a
        python3 signalsweep.py "..."
    """
...
YT_DATA_API_SEARCH_URL = "https://www.googleapis.com/youtube/v3/search"


def _trend_proxy_search(
    topic: str,
    api_key: str,
    from_date: str,
) -> List[Dict[str, Any]]:
    """Trend-proxy search via YouTube Data API v3 search.list with viewCount ordering.

    Compensates for the deprecated mostPopular chart (Jul 2025, now Music/Movies/Gaming only)
    by searching for recent high-view-count videos matching the topic.

    Expensive: 100 quota units per call (vs 1 for videos.list). Only call at deep depth.

    Args:
        topic: Search topic
        api_key: YouTube Data API v3 key
        from_date: Start date (YYYY-MM-DD) — used to derive 7-day recency window

    Returns:
        List of item dicts in the same shape as search_youtube() output,
        each with ``metadata.trend_proxy: True``.
    """
    if not api_key:
        return []
    ...
    # publishedAfter = 7 days ago in RFC 3339
    try:
        base_date = datetime.strptime(from_date, "%Y-%m-%d").replace(tzinfo=timezone.utc)
    except (ValueError, TypeError):
        base_date = datetime.now(timezone.utc)
    seven_days_ago = (base_date - timedelta(days=7) if base_date > datetime.now(timezone.utc) - timedelta(days=7)
                      else base_date)
    published_after = seven_days_ago.strftime("%Y-%m-%dT00:00:00Z")
    ...
    url = f"{YT_DATA_API_SEARCH_URL}?{query_string}"

    _log(f"Trend-proxy search for '{core_topic}' (publishedAfter={published_after})")
    ...
    try:
        ...
        _log(f"Trend-proxy search failed: {exc}")
        return []

    api_items = data.get("items", [])
    if not api_items:
        _log("Trend-proxy search returned 0 results")
        return []
    ...
    for entry in api_items:
        ...
        items.append({
            ...
            "date": date_str,
            "engagement": {
                "views": 0,  # search.list snippet doesn't include view counts
                "likes": 0,
                "comments": 0,
            },
            "duration": None,
            "relevance": _compute_relevance(core_topic, f"{title} {description}"),
            "why_relevant": f"YouTube trend-proxy: {title[:60]}",
            "description": description,
            "metadata": {"trend_proxy": True},
        })

    _log(f"Trend-proxy search found {len(items)} videos")
    return items
...
def search_and_transcribe(
    ...
    # Step 1b: At deep depth, merge trend-proxy results from YouTube Data API
    # search.list with viewCount ordering. This compensates for the deprecated
    # mostPopular chart. Costs 100 quota units — only fires at deep depth.
    if depth == "deep":
        api_key = os.environ.get("YOUTUBE_API_KEY", "").strip()
        if api_key:
            trend_items = _trend_proxy_search(topic, api_key, from_date)
            for item in trend_items:
                vid = item.get("video_id", "")
                if vid and vid not in seen_ids:
                    seen_ids.add(vid)
                    items.append(item)
                # If already seen from regular search, regular result takes priority
                # (no trend_proxy flag set on existing items)
