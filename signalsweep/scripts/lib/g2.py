"""G2 SaaS product review scrape (signalsweep 3.17.0+).

Fetches `g2.com/products/{slug}/reviews` HTML and parses embedded JSON-LD
review data. Companion to `capterra.py` for B2B software research.

Tier: v3.7 `SIGNALSWEEP_DISABLE_PAID_APIS`. No credentials. 500ms polite-sleep.

Topic: G2 product slug (e.g., `hubspot-marketing-hub`) or full G2 URL.

Docs: g2.com (DevTools inspection).
"""
...
def _normalize_slug(topic: str) -> str:
    ...
    if t.startswith("www."):
        t = t[4:]
    if t.startswith("g2.com/products/"):
        t = t[len("g2.com/products/"):]
    return t.split("/", 1)[0]
...
def search_g2(
    ...
    return {"items": {"slug": slug, "html": html}, "error": None}


def parse_g2_response(
    ...
    if not json_ld_blocks:
        sys.stderr.write("[g2] no JSON-LD blocks found; G2 may have changed layout\n")
        return []
    ...
    if not review_list and aggregate_count > 0:
        item = reviews.build_review_item(
            ...
            relevance=0.7,
            why_relevant=f"G2 aggregate for {product_name}",
            extra_metadata={"slug": slug, "category": "saas"},
        )
        return [item]
    ...
    for i, r in enumerate(review_list[:limit]):
        ...
        review_id = r.get("@id") or f"{slug}-{i}"

        item = reviews.build_review_item(
            item_id=f"g2:{review_id}",
            platform="g2",
            product_name=product_name,
            url=f"https://www.g2.com/products/{slug}/reviews",
            source_domain="g2.com",
            ...
            relevance=0.7,
            why_relevant=f"G2 review for {product_name}",
            extra_metadata={"slug": slug, "category": "saas"},
        )
        parsed.append(item)
    return parsed
