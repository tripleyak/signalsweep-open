...
def _render_badge() -> list[str]:
    ...
    return [
        f"🌐 signalsweep v{version} · synced {today}",
        "",
    ]
...
SOURCE_LABELS = {
    ...
    "grok_deepsearch": "Grok DeepSearch",
    "openrouter_research": "OpenRouter Research",
    "x_sc": "X (ScrapeCreators)",
    "rss_blogs": "RSS Blogs",
    "sec_edgar": "SEC EDGAR",
    "sec_xbrl": "SEC XBRL",
    "fda_openfda": "FDA openFDA",
    ...
    "wayback_machine_cdx": "Wayback Machine",
...
def render_compact(report: schema.Report, cluster_limit: int = 8, fun_level: str = "medium", save_path: str | None = None) -> str:
    ...
    lines = [
        *_render_badge(),
        f"# signalsweep v{_skill_version()}: {report.topic}",
        "",
...
def _render_canonical_boundary() -> list[str]:
    ...
    return [
        "",
        "---",
        "# END OF signalsweep CANONICAL OUTPUT",
        "",
...
def _render_pre_research_warning(report: schema.Report) -> list[str]:
    ...
    return [
        ...
        f"then rerun `/signalsweep {report.topic}`. The skill will resolve handles",
...
def _render_degraded_run_warning(report: schema.Report) -> list[str]:
    ...
    return [
        ...
        "Hermes, Gemini, or any agent runtime that invoked `/signalsweep`): YOU ARE",
...
def render_comparison_multi(
    ...
    *,
    ...
    lines: list[str] = [
        *_render_badge(),
        f"# signalsweep v{_skill_version()}: {synthesized_topic}",
        "",
        *_assistant_safety_lines(),
...
def render_full(report: schema.Report) -> str:
    ...
    # self-describing. The artifact is populated by signalsweep.py's
...
def render_research(report: schema.Report) -> str:
    """Render full deep-research synthesis bodies and citations per provider."""
    lines = [
        f"# signalsweep deep research: {report.topic}",
        "",
        f"- Date range: {report.range_from} to {report.range_to}",
    ...
    deep_research_sources_with_items = [
        (source, items)
        for source, items in sorted(report.items_by_source.items())
        if source in DEEP_RESEARCH_PROVIDERS and items
    ]

    if not deep_research_sources_with_items:
        lines.extend([
            "## No deep-research syntheses available",
            "",
            "Pass `--deep-research` and request providers via `--search=`, e.g.:",
            "",
            "    signalsweep \"<topic>\" --deep-research --search=claude_research,chatgpt_deep_research",
            "",
            "Available deep-research providers: chatgpt_deep_research, claude_research, "
            "gemini_deep_research, grok_deepsearch, openrouter_research, perplexity.",
            "",
        ])
    else:
        for source, items in deep_research_sources_with_items:
            synthesis_item = None
            citation_items = []
            for item in items:
                if item.metadata.get("provider") == source or item.metadata.get("synthesis"):
                    synthesis_item = item
                else:
            ...
            if synthesis_item:
                meta = synthesis_item.metadata or {}
                full_synthesis = meta.get("synthesis") or synthesis_item.body or synthesis_item.snippet
                ...
                if cost is not None:
                    meta_bits.append(f"~${cost:.2f}/query")
                if meta_bits:
                    lines.append(f"_{' · '.join(meta_bits)}_")
                    lines.append("")
                lines.append(full_synthesis)
                lines.append("")
            else:
                lines.append("_(no synthesis returned)_")
                lines.append("")
            ...
            if citation_items:
                ...
                for cit in citation_items:
                    ...
                    lines.append(f"- [{title}]({cit.url})" if cit.url else f"- {title}")
                lines.append("")

    other_sources = [
        source
        for source, items in sorted(report.items_by_source.items())
        if source not in DEEP_RESEARCH_PROVIDERS and items
    ]
    if other_sources:
        lines.extend([
            "## Other sources also active",
            "",
            ", ".join(_source_label(s) for s in other_sources),
            "",
            "_For ranked-cluster output across all sources, use `--emit=compact` or `--emit=context`._",
            "",
        ])
