...
def ensure_supported_python(version_info: tuple[int, int, int] | object | None = None) -> None:
    ...
    if (major, minor) >= MIN_PYTHON:
        return
    sys.stderr.write(
        "signalsweep v3 requires Python 3.12+.\n"
...
from lib import deep_research, env, html_render, pipeline, render, schema, ui
...
def save_output(
    ...
    elif emit in {"json", "html", "research"}:
...
def emit_output(
    ...
    if emit == "research":
        return render.render_research(report)
...
def read_synthesis_file(path: str) -> str:
    try:
        ...
        sys.stderr.write(f"[signalsweep] Cannot read --synthesis-file: {exc}\n")
        raise SystemExit(2)
...
def persist_report(report: schema.Report) -> dict[str, int]:
    try:
        ...
        raise RuntimeError(
            "Local SQLite store support is not included in this public package. "
            "Use the full repository build to enable --store."
        ) from exc
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--emit",
        default="compact",
        choices=["compact", "json", "context", "md", "html", "research"],
        help=(
            "Output mode. 'research' surfaces full deep-research synthesis bodies "
            "per provider (use with --deep-research)."
        ),
    )
    ...
    parser.add_argument("--dark-demand", action="store_true",
                        help="Expand queries with dark-demand language (friction, absence, workaround, ritual patterns)")
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="N",
        help="Auto-discover N competitor entities and fan out signalsweep across all of them as a comparison (default N=2 → 3-way: original + 2 peers; range 1..6). Use --competitors-list to override discovery.",
    )
    parser.add_argument(
        "--competitors-list",
...
def main() -> int:
    ...
    if topic.lower() == "setup":
        ...
        from_browser = results.get("from_browser") or "auto"
    ...
    if args.synthesis_file:
        if args.emit == "html":
            ...
            sys.stderr.write("[signalsweep] Warning: --synthesis-file is only used with --emit=html; ignoring.\n")
    ...
    try:
        ...
        # Auto-resolve: use configured web-search backends to discover
        # subreddits/handles before planning. This is the engine-side equivalent
        # of SKILL.md Steps 0.55/0.75 for hosts without native web search.
        ...
        # v3.8 provider envelopes are cost-gated behind this flag.
        v38_deep_research_providers = {
        ...
        if args.deep_research:
            if not deep_research.is_deep_research_enabled(config):
                print(
                    "Error: --deep-research is disabled by SIGNALSWEEP_DISABLE_DEEP_RESEARCH "
                    "(public mode defaults it on). Set SIGNALSWEEP_DISABLE_DEEP_RESEARCH=0 to opt in.",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            requested_dr = set(requested_sources) & v38_deep_research_providers
            if requested_dr:
                sys.stderr.write(
                    f"[signalsweep] WARNING: deep-research provider(s) "
                    f"{sorted(requested_dr)} requested in --search= but "
                    f"--deep-research flag not passed. These sources will NOT run "
                    f"(opt-in cost protection). Add --deep-research to enable.\n"
                )

        if args.dark_demand:
            config["_dark_demand"] = True
            ...
            if comp_explicit:
                ...
                if not resolve_mod._has_backend(config) and not args.mock:
                    sys.stderr.write(
                        ...
                        "Hermes, Gemini, any agent with a web-search capability): YOU have "
                        "web search. Use it to run full Step 0.55 per entity, then invoke "
                        ...
                        "  3. Re-invoke: /signalsweep '{topic} vs {peer1} vs {peer2}' "
    ...
    # ~/.config/signalsweep/.env, which env.py loads but does not propagate
    ...
    if args.store or _store_env in ("1", "true", "yes"):
        ...
        sys.stderr.write(
            f"[signalsweep] Stored {counts['new']} new, {counts['updated']} updated findings\n"
        )
        sys.stderr.flush()
    ...
    try:
        ...
        research_results = {
            ...
            "active_sources": [source for source, items in report.items_by_source.items() if items],
        }
    ...
    if args.save_dir:
        ...
        sys.stderr.write(f"[signalsweep] Saved output to {save_path}\n")
        ...
        if entity_reports and len(entity_reports) > 1:
            for label, entity_report in entity_reports[1:]:
                ...
                sys.stderr.write(f"[signalsweep] Saved output to {peer_path}\n")
        sys.stderr.flush()
    print(rendered)
    return 0
